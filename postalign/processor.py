...
class Processor(Generic[ReturnType]):  # noqa: UP046
    command_name: str
    is_output_command: bool
    _processor: Callable[[Iterable[RefSeqPair], list[Message]], ReturnType]

    def __init__(
        self,
        command_name: str,
        is_output_command: bool,
        processor: Callable[[Iterable[RefSeqPair], list[Message]], ReturnType],
    ) -> None:
...
def output_processor(
    ...
    def wrapper(
        processor: Callable[[Iterable[RefSeqPair], list[Message]], Iterable[str]],
    ) -> Processor[Iterable[str]]:
...
def intermediate_processor(
    command_name: str,
) -> Callable[
    [Callable[[Iterable[RefSeqPair], list[Message]], Iterable[RefSeqPair]]],
