...
def group_by_codons(
    ...
    bp = -1
    for refna, seqna in zip(refnas, seqnas, strict=False):
        if not refna.is_gap:
            bp = (bp + 1) % 3
            if bp == 0:
                # begin new codon
...
def find_codon_trim_slice(codons: list[list[NAPosition]]) -> slice:
    left_trim: int = 0
...
def group_by_gene_codons(
    ...
    results: list[tuple[str, list[list[NAPosition]], list[list[NAPosition]]]] = []
