...
def _translate_codon(
    ...
    if del_as and len_nas == 3 and nas == GAP_CODON:
        ...
        return del_as
    if fs_as and (len_nas < 3 or GAP_NA in nas):
...
def translate_codons(
    ...
    for codon in chunked(nas_bytes, 3):
        aas: tuple[int, ...] = _translate_codon(tuple(codon), fs_as_tuple, del_as_tuple)
...
def compare_codon(base: bytes, target: bytes) -> bool:
    ha: int
    sna: int
    tna: int
    ...
    for ha in b'BDHVN':
        if ha in target:
            ...
            return False
    for sna, tna in zip(base, target, strict=False):
        if tna in b'ACGT':
            if tna != sna:
                break
        else:
