"""Optimized codon alignment (T2).

Same codon_align() API as codon_alignment.py but with algorithmic
improvements (Option D from the refactor plan):

D1 — Precompute other-side amino acids; avoid redundant translate_codons
D4 — Search from original gap position outward (center-expand)
     with leftmost-index tie-breaking to match T1 output exactly
"""
...
# -----------------------------------------------------------------------
# Unchanged helpers (same as original)
# -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
# D1 — Optimized scoring: precompute other_aas, avoid redundant work
# -----------------------------------------------------------------------
...
def calc_match_score_precomputed(
    ...
    """Like calc_match_score but reuses precomputed other_aas."""
    myna: NAPosition
    otherna: NAPosition
    myaa: bytes
    otheraa: bytes
...
def center_expand_positions(
    ...
    """Generate search positions starting from center, expanding outward.

    Returns a list of valid indices in center-expand order.
    """
    positions: List[int] = []
...
# -----------------------------------------------------------------------
# D1 + D4 — Optimized find_best_matches
# -----------------------------------------------------------------------
