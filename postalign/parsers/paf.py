...
def load(
    ...
    paf_lookup: dict[str, set[tuple[int, int, int, int, str]]] = defaultdict(set)
    ...
    for seq in seqs:
        try:
            pafs: set[tuple[int, int, int, int, str]] = paf_lookup[str(seq.seqid)]
        except KeyError:
            # alignment for sequence is not found
            yield (
                refseq.push_seqtext([], modtext='error()', start_offset=0),
        ...
        for ref_start, ref_end, seq_start, seq_end, cigar_text in sorted(
            pafs, key=lambda x: (x[1], x[0]), reverse=True
        ):
