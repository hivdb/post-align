...
class Modifier:
    ...
    def __add__(self: 'Modifier', other: 'Modifier') -> 'Modifier':
        ...
        if (
            self.slicetuples
            and other.slicetuples
            and len(self.parent_mods) == len(other.parent_mods)
            and self.parent_mods[0]() == other.parent_mods[0]()
        ):
            ...
            merged = Modifier(f'concat(step{self.step},step{other.step})')
...
class ModifierLinkedList:
    ...
    def __str__(self: 'ModifierLinkedList') -> str:
        ...
        for step, group in self:
            text_buffer.append('{}:{}'.format(step, ','.join(str(m) for m in group)))
    ...
    def __iter__(
        ...
        return groupby(sorted(all_mods, key=get_step), get_step)
